...
function respond() {
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    var req = request.text.substring(5,request.text.length);
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/sebl/players?name=&position=&"+rep+");
    this.res.end();
  }
